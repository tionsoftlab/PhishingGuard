...
const ExpertNewsWrite: React.FC = () => {
    ...
    return (
      <div className="max-w-4xl mx-auto pb-20 md:pb-20 p-4 md:p-8">
        <div className="text-center py-20">
          ...
          <p className="text-neutral-500 dark:text-neutral-400 mb-2 font-medium">전문가만 뉴스를 작성할 수 있습니다.</p>
          <p className="text-neutral-400 dark:text-neutral-500 text-sm mb-6">전문가 인증을 완료한 후 다시 시도해주세요.</p>
          <button
            onClick={() => router.back()}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push(`/experts/${data.newsId}`);
      } else {
        const err = await response.json();
        alert(err.error || '뉴스 작성에 실패했습니다.');
      }
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="보안 뉴스 본문을 작성하세요. 전문적인 분석과 대응 방안을 포함해주세요."
              rows={18}
              className="w-full px-4 py-3 border border-neutral-200 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 text-neutral-900 dark:text-white placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
            />
...
export default ExpertNewsWrite;
