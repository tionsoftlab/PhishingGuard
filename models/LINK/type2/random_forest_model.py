...
print("데이터 로드 중...")
df = pd.read_csv('dataset_type2.csv')
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
            xticklabels=['정상', '피싱'],
            yticklabels=['정상', '피싱'])
plt.title('혼동 행렬 (Confusion Matrix) - Random Forest')
plt.ylabel('실제 값')
plt.xlabel('예측 값')
plt.tight_layout()
plt.savefig('rf_confusion_matrix.png', dpi=300, bbox_inches='tight')
print("혼동 행렬 저장: rf_confusion_matrix.png")

plt.figure(figsize=(10, 8))
top_features = feature_importance.head(15)
sns.barplot(data=top_features, x='importance', y='feature', palette='viridis')
plt.title('특징 중요도 (Feature Importance) - Top 15')
plt.xlabel('중요도')
plt.ylabel('특징')
plt.tight_layout()
plt.savefig('rf_feature_importance.png', dpi=300, bbox_inches='tight')
print("특징 중요도 저장: rf_feature_importance.png")
...
with open('type2_rf_results.json', 'w', encoding='utf-8') as f:
    json.dump(results, f, indent=2, ensure_ascii=False)

print("\n결과 저장 완료: type2_rf_results.json")
